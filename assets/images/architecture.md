...
- This solution comprises **four** containers. There are no ISV partner containers.
- The canonical embeddable architecture image referenced by the top-level README is
  `assets/images/architecture.svg`. No draw.io source (`.drawio`) or draw.io-exported
  SVG (`.drawio.svg`) is published under `assets/`.
