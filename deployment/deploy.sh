...
if [[ "${UI_ONLY}" -eq 1 ]]; then
  ...
  NLB_DNS="$(kubectl get svc orchestrator -n artf -o jsonpath='{.status.loadBalancer.ingress[0].hostname}' 2>/dev/null || echo '')"
  if [[ -z "${NLB_DNS}" ]]; then
...
# Ensure the artf namespace exists
kubectl create namespace artf --dry-run=client -o yaml | kubectl apply -f -
...
kubectl rollout status deployment/orchestrator -n artf --timeout=120s || true
...
if [[ -z "${DEMO_USER_EMAIL:-}" ]]; then
  # Try to get the caller's email from IAM alias or default to a valid placeholder
  _CALLER_ALIAS="$(aws sts get-caller-identity --query UserId --output text 2>/dev/null | cut -d: -f2)"
  if [[ -n "${_CALLER_ALIAS}" && "${_CALLER_ALIAS}" != "None" && "${_CALLER_ALIAS}" == *"@"* ]]; then
    DEMO_USER_EMAIL="${_CALLER_ALIAS}"
  else
...
if [[ -n "${COGNITO_USER_POOL_ID}" ]]; then
  if aws cognito-idp admin-get-user --user-pool-id "${COGNITO_USER_POOL_ID}" --username "${DEMO_USER_EMAIL}" --region "${AWS_REGION}" >/dev/null 2>&1; then
    log "  Demo user exists: ${DEMO_USER_EMAIL}"
  else
    # The temporary password is supplied by the operator at deploy time via the
    # DEMO_USER_PASSWORD environment variable (no password is hardcoded here).
    if [[ -z "${DEMO_USER_PASSWORD:-}" ]]; then
      log "  DEMO_USER_PASSWORD is not set; skipping demo user creation."
      log "  Set DEMO_USER_PASSWORD to a temporary password that meets the Cognito password policy and re-run to create the demo user."
    else
      aws cognito-idp admin-create-user \
        ...
        --region "${AWS_REGION}" >/dev/null
      log "  Created demo user: ${DEMO_USER_EMAIL} (temporary password supplied via DEMO_USER_PASSWORD)."
      log "  You will be prompted to set a permanent password on first login."
    fi
  fi
fi
